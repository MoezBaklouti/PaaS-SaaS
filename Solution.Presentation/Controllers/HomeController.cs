        ...
        int autoId = 0;
            ...
            switch (s)
            {
                case "5": // school voting
                    ...
                    var isIt = MyContext.VoteVM.Where(v => v.SectionId == sectionId &&
                    ...
                    var sch = MyContext.SchoolVM.Where(sc => sc.AutoId == autoId).FirstOrDefault();
                    if (sch != null)
                    {
                    ...
                    MyContext.Entry(sch).State = EntityState.Modified;
                        sch.Votes = updatedVotes;
                    MyContext.VoteVM.SaveChanges();

                        VoteVM vm = new VoteVM()
                        {
                            Active = true,
